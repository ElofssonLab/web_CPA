...
ma <- function(x,n=5000){filter(x,rep(1/n,n), sides=2)}
...
cairo_ps(file = "figures/meff-pcons.eps", onefile = FALSE, fallback_resolution = 600)
plot(log10(structure$Meff),structure$pcons,,cex=0.6,col = alpha("lightblue", 0.4),xlab="log(Meff)",ylab="Pcons")
points(log10(nostruct$Meff),nostruct$pcons,cex=0.6,pch=0,col = alpha("pink", 0.4))
points(log10(structure$Meff[order(structure$Meff)]),ma(structure$pcons[order(structure$Meff)]), col=alpha("blue",0.2))
points(log10(nostruct$Meff[order(nostruct$Meff)]),ma(nostruct$pcons[order(nostruct$Meff)]), col=alpha("red",0.2))
